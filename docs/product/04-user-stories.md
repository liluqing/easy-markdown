...
- 新建或重命名遇到同名目标时停止，不覆盖目标。
- 删除前确认，且只进入系统回收站；失败时原文件保持不变。
- 当前文档有未保存内容或外部冲突时，先保存或解决冲突，不能直接删除并丢弃内容。
...
- 停止输入约 800 ms 后内容落盘；切换文档、重命名或正常关闭前尝试立即保存。
- 保存失败时显示持续可见的错误状态。
- 关闭并重新打开应用后内容仍存在。
- 自动保存不自动创建 Git Commit。
- 强制终止应用可能丢失最近约 800 ms 尚未落盘的输入，但不得破坏最后一次成功保存的文件。
...
- 存在未保存内容时不静默覆盖。
- 用户可以比较本地编辑内容和外部内容。
- 脏文档被外部删除或移动到未知位置时保留草稿并停止自动保存；仅在用户明确确认后原路径重建。
